...
data.biomass = merge(data.biomass, tnc.final[,c("Date", "T_treatment", "chamber_type", "TNC_tot", "TNC_tot_SE", "TNC_leaf", "TNC_leaf_SE", "TNC_wood", "TNC_wood_SE", "TNC_root", "TNC_root_SE")], all=TRUE)

data.GPP.Ra.LA.mass = merge(data, data.biomass, by=c("Date","T_treatment","chamber_type"), all=TRUE)

pd <- position_dodge(2)
p3 = ggplot(data.biomass[complete.cases(data.biomass$TNC_leaf),], aes(x=Date, y=TNC_leaf, group = interaction(T_treatment,chamber_type), colour=T_treatment, shape=chamber_type)) + 

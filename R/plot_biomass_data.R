...
data.biomass = subset(data.biomass, Date >= as.Date("2014-02-18") & Date <= as.Date("2014-05-27"))
...
plots[[3]] = ggplot(data.biomass, aes(x=Date, y=LM, group = interaction(T_treatment,chamber_type), colour=T_treatment, shape=chamber_type)) + 
  geom_point(position=pd) +

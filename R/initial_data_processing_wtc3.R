...
#----------------------------------------------------------------------------------------------------------------
# Script to setup a model for the leaf litter
...
plot = ggplot(rootmass.harvest.ratio.melt, aes(x=Date, y=value, group = variable, colour=variable)) +
  # geom_point(position=pd, size=0.5) +
  # geom_errorbar(position=pd, aes(ymin=value-SE, ymax=value+SE), colour="grey", width=1) +
  geom_ribbon(data = rootmass.harvest.ratio.melt, aes(ymin=value-SE, ymax=value+SE), linetype=2, alpha=0.1,size=0.1) +
  geom_line(position=pd,data = rootmass.harvest.ratio.melt, aes(x = Date, y = value, group = variable, colour=variable)) +
  ylab("Rootmass ratio") + xlab("") +
  scale_x_date(date_labels="%b %y",date_breaks  ="1 month",limits = c(min(Rd$Date)-2, max(Rd$Date)+2)) +
  labs(colour="Rootmass types") +
  theme_bw() +
  theme(legend.title = element_text(colour="black", size=font.size)) +
  theme(legend.text = element_text(colour="black", size=font.size)) +
...
png("output/4.Rootmass_ratio.png", units="px", width=2000, height=1000, res=180)
plot
dev.off()
#----------------------------------------------------------------------------------------------------------------
...
#   theme(legend.title = element_text(colour="black", size=font.size)) +
#   theme(legend.text = element_text(colour="black", size=font.size)) +
#   theme(legend.position = c(0.2,0.84), legend.box = "horizontal") + theme(legend.key.height=unit(0.8,"line")) +
#   theme(legend.key = element_blank()) +
#   theme(text = element_text(size=font.size)) +
#   theme(axis.title.x = element_blank()) +
#   theme(axis.title.y = element_text(size = font.size, vjust=0.3)) +
...
# # plot
# # dev.off()
# png("output/4.Biomass_ratio.png", units="px", width=2000, height=1000, res=180)
# plot
# dev.off()
#----------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------
...
plots[[2]] = ggplot(data = resp.fol.final, aes(x=Tair_al.mean, y=Rmass.mean)) + 
  ...
  ylab(expression(R[leaf25]~(nmol~CO[2]~g^-1~s^-1))) + xlab(expression("Previous 3-day mean daily Tair"~(degree*C))) + 
...
plots[[3]] = ggplot(data = resp.fol.final, aes(x=Tair_al.mean, y=Rmass.mean)) + 
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE) +
  geom_smooth(method="lm",se=FALSE) + geom_point(size=0.75) + 
  ylab(expression(R[leaf25]~(nmol~CO[2]~g^-1~s^-1))) + xlab(expression("Previous 7-day mean daily Tair"~(degree*C))) + 
  theme_bw() + theme(legend.key.width=unit(0.9,"line")) 
...
plots[[4]] = ggplot(data = resp.fol.final, aes(x=Tair_al.mean, y=Rmass.mean)) + 
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE) +
  geom_smooth(method="lm",se=FALSE) + geom_point(size=0.75) + 
  ylab(expression(R[leaf25]~(nmol~CO[2]~g^-1~s^-1))) + xlab(expression("Previous 1-day mean daily Tair"~(degree*C))) + 
  theme_bw() + theme(legend.key.width=unit(0.9,"line")) 
...
plots[[5]] = ggplot(data = resp.fol.final, aes(x=Tair_al.mean, y=Rmass.mean)) + 
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE) +
  geom_smooth(method="lm",se=FALSE) + geom_point(size=0.75) + 
  ylab(expression(R[leaf25]~(nmol~CO[2]~g^-1~s^-1))) + xlab(expression("3-day mean daily Tair"~(degree*C))) + 
  theme_bw() + theme(legend.key.width=unit(0.9,"line")) 
...
plots[[6]] = ggplot(data = resp.fol.final, aes(x=Tair_al.mean, y=Rmass.mean)) + 
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE) +
  geom_smooth(method="lm",se=FALSE) + geom_point(size=0.75) + 
  ylab(expression(R[leaf25]~(nmol~CO[2]~g^-1~s^-1))) + xlab(expression("Mean daily Tair"~(degree*C))) + 
  theme_bw() + theme(legend.key.width=unit(0.9,"line")) 
...
# calculate daily foliage respiration rates
met.data[,"Rd.foliage"] = with(met.data, met.data[,"rd25.foliage"] * q10^((Tair_al-25)/10)) # unit (gC per gC root per day)

Rd <- summaryBy(Rd.fineroot+Rd.intermediateroot+Rd.coarseroot+Rd.boleroot+Rd.foliage ~ Date+T_treatment, data=met.data, FUN=mean, keep.names=TRUE , na.rm=TRUE) # Sum of all same day Rd

# add daily stem and branch respiration rates
Rd$Rd.branch = Rd$Rd.coarseroot
Rd$Rd.stem = Rd$Rd.boleroot
...
font.size = 15
plot = list() 
meas = as.factor(c("Rd.fineroot","Rd.intermediateroot","Rd.coarseroot","Rd.boleroot","Rd.foliage"))
...
for (p in 1:length(meas)) {
  ...
  if (p==5) {
    plot[[i]] = plot[[i]] + ylab(expression(R[d(foliage)]~"(g C "*g^"-1"*" C "*d^"-1"*")"))
  }
}
